...
public class SenderIdSelectorTest {
  ...
  private static Phonenumber.PhoneNumber MX = PhoneNumberUtil.getInstance().getExampleNumber("MX");
  private static Phonenumber.PhoneNumber US = PhoneNumberUtil.getInstance().getExampleNumber("US");

  @Test
  public void defaultSender() {
    final SenderIdSelector selector = new SenderIdSelector(
        new MessageBirdConfiguration("test", "default", Map.of("mx", "mx-sender")));
