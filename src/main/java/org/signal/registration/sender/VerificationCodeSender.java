 ...
 * so, senders should return a {@link String} containing any data that needs to be preserved from the
 * {@link #sendVerificationCode(Phonenumber.PhoneNumber, List, ClientType)} method; that string will be provided later to
 * the {@link #checkVerificationCode(String, byte[])} method when a client provides a verification code in the context
 * of the same registration session.
 */
public interface VerificationCodeSender {

  /**
   * Returns the type of message transport supported by this sender.
   *
   * @return the type of message transport supported by this sender
   */
  ...
  /**
   * Indicates whether this sender can deliver messages to the given phone number using any of the given languages and
   ...
   */
  boolean supportsDestination(Phonenumber.PhoneNumber phoneNumber,
   ...
   */
  CompletableFuture<byte[]> sendVerificationCode(Phonenumber.PhoneNumber phoneNumber,
      List<Locale.LanguageRange> languageRanges,
      ClientType clientType);

  /**
   * Checks whether the verification code provided by a client matches the verification code sent via an earlier call to
   * {@link #sendVerificationCode(Phonenumber.PhoneNumber, List, ClientType)}.
   *
   * @param verificationCode the verification code provided by a client
   * @param sessionData the data returned by {@link #sendVerificationCode(Phonenumber.PhoneNumber, List, ClientType)}
